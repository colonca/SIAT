        ...
        <div class="card">
            <div class="header container">
                <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                    <h2>
                        Docentes de Permanencia del Sistema
                    </h2>
                </div>
            ...
            <div class="body table-responsive table-bordered">
                <table class="table table-condensed" id="docentes_permanencia">
                    ...
                    <tbody>
                       @foreach($docentePermanencia as $docente)
                         <tr>
                             <td>{{$docente->cedula}}</td>
                             <td>{{$docente->primer_nombre}} {{$docente->primer_apellido}}</td>
                             <td>{{$docente->direccion}} {{$docente->barrio}}</td>
                             <td>
                                 <ul class="list-unstyled">
                                     <li>{{$docente->email}}</li>
                                     <li>{{$docente->email_institucional}}</li>
                                 </ul>
                             </td>
                             <td>
                                 <ul class="list-unstyled">
                                     <li>{{$docente->telefono}}</li>
                                     <li>{{$docente->celular}}</li>
                                 </ul>
                             </td>
                             <td>
                                 <ul class="list-unstyled">
                                     <li>{{$docente->contacto->nombre}}</li>
                                     <li>{{$docente->contacto->parentezco}}</li>
                                     <li>{{$docente->contacto->celular}}</li>
                                 </ul>
                             </td>
                             <td>
                                 <a href="{{route('docentes_permanencia.show',$docente->id)}}"  class="btn btn-sm btn-info" title="Ver Informacion">
                                     <i class="material-icons">
                                         remove_red_eye
                                     </i>
                                 </a>

                                 <a href="#" onclick="eliminar(event,{{$docente->id}})"  class="btn btn-sm btn-danger" title="Eliminar" >
                                     <i class="material-icons">
                                         delete
                                     </i>
                                 </a>

                                 <a href="{{route('docentes_permanencia.edit',$docente->id)}}" class="btn btn-sm btn-warning margin" title="Actualizar">
                                     <i class="material-icons">
                                         update
                                     </i>
                                 </a>

                                 <a href="#"  class="btn btn-sm btn-info" onclick="verHorario({{$docente->id}})"  title="Ver Horario">
                                     <i class="material-icons">
                                         date_range
                                     </i>
                                 </a>
    ...
    <script>
        ...
        function eliminar(event,id){
            ...
            Swal.fire({
                ...
                if (result.value) {
                    let url = 'docentes_permanencia/'+id;
                    axios.delete(url).then(result => {
                        Swal.fire(
                            'Eliminado!',
                            'el docente se a eliminado correctamente.',
                            'success'
                        ).then(result => {
                            location.reload();
                        });
